...
class BigramTrainer(object):
    ...
    def stats(self):
        ...
        # unigram
        for i,t in self.word.items():
            rows_to_print.append(f"{i} {t} {self.unigram_count[i]}")
        # bigram
        for i1,item in self.bigram_count.items():
            for i2,count in item.items():
                rows_to_print.append(f"{i1} {i2} {math.log(count/self.unigram_count[i1]):.15f}")
